...
using System.Linq;
using TaskFive.DTO;
using TaskFive.Entity;
using TaskFive.FactoryMethod;
...
namespace TaskFive
{
	...
	{
		private static FactoryDao _daoCreate = new FactoryDao();
		...
		/// </summary>
		/// <returns></returns>
		public static IEnumerable<BookWithUseDto> CalculateHowManyTimesBooksWasTaken()
		{
			...
			var linqRequest = from report in reportDtos
							  ...
							  select new BookWithUseDto() { Book = book.Key, Count = book.Count() };
			return linqRequest;
